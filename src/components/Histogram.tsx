...
type OptionsProps = {
  ...
  xAxisName: string,
  yAxisName: string,
  aggregationData: any, // eslint-disable-line @typescript-eslint/no-explicit-any
  value: string,
  labelFormatter: (...args: any[]) => any, // eslint-disable-line @typescript-eslint/no-explicit-any
}
...
const GetHistogramChartOptions = ({
  ...
  const xAxisData = aggregationData.map((item: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
    value: item.key_as_string,
  }));
  let startIndex = -1;
  let endIndex = -1;
  if (value && Array.isArray(value)) {
    startIndex = xAxisData.findIndex((i: any) => i.value === value[0]); // eslint-disable-line @typescript-eslint/no-explicit-any
    endIndex = xAxisData.findIndex((i: any) => i.value === value[1]); // eslint-disable-line @typescript-eslint/no-explicit-any
  }
  return {
    title: chartTitle,
    toolbox: {
    ...
    series: [
      {
        type: 'bar',
        data: aggregationData.map((item: any) => ({ // eslint-disable-line @typescript-eslint/no-explicit-any
